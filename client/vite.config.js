    ...
    server: {
      ...
      proxy: {
        ...
        '/product-images-local': {
          ...
          secure: false,
          timeout: 120000,
          rewrite: (path) => path.replace(/^\/product-images-local/, '/products/image-mirror'),
        },
        '/audit-local': {
          target: chatTarget,
          changeOrigin: true,
          secure: false,
          timeout: 60000,
          rewrite: (path) => path.replace(/^\/audit-local/, '/audit/log'),
        },
        '/platform-local': {
          target: chatTarget,
          changeOrigin: true,
          secure: false,
          timeout: 60000,
          rewrite: (path) => path.replace(/^\/platform-local/, '/platform'),
        },
        '/payment-local': {
          target: chatTarget,
          changeOrigin: true,
          secure: false,
          timeout: 60000,
          rewrite: (path) => path.replace(/^\/payment-local/, '/payment'),
        },
        '/brandhive-api': {
          target: apiTarget,
          changeOrigin: true,
